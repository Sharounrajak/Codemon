...
export default async function SnippetCard({ snippet }) {
    ...
    const userSnippets = await snippet.find({ userId: "user-123", isDemo: false });
 ...
 * @name handleCopy
 */
/*******  3cca2cd9-89e4-4f7b-be79-66acb7140ee9  *******/
    const handleCopy = () => {
    ...
    >
      <CardContent sx={{ flexGrow: 1, p: 3 }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
          <Avatar
            sx={{
              bgcolor: getLanguageColor(userSnippets.language),
              width: 32,
              height: 32,
              mr: 2,
              fontSize: "0.875rem",
          ...
          <Box sx={{ flexGrow: 1 }}>
            ...
            <Typography variant="caption" sx={{ color: "#cbd5e1" }}>
              {userSnippets.createdAt}
            </Typography>
          </Box>
          <Chip
            label={userSnippets.language}
            size="small"
            sx={{
              backgroundColor: getLanguageColor(userSnippets.language),
              color: "white",
              fontWeight: 500,
              fontSize: "0.75rem",
            }}
          ...
          {isExpanded ? (
            ...
            <Box sx={{ mb: 2 }}>
              <Editor
                height="300px"
                width="780px"
                language={userSnippets.language}
                value={userSnippets.code}
                theme="vs-dark"
                options={{
                  readOnly: true,
        ...
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mb: 2 }}>
          {userSnippets.tags.map((tag, index) => (
            <Chip
              key={index}
              label={`#${tag}`}
              size="small"
